...
cloud = session.connect_scratch_cloud(os.getenv("PROJECT_ID"))
events = cloud.events()
encode_map = {
...
def encode(text: str) -> str:
    """
    Convert text to a sequence of triplets.
    """
    encoded = ""
    for char in text:
        if char in encode_map:
            ...
            raise ValueError(f"Scratch => Character '{char}' not in encoding map.")
    return encoded
...
def decode(triplets: str) -> str:
    if len(triplets) % 3 != 0:
        raise ValueError("Scratch =>Triplets string length must be a multiple of 3.")
...
def ask_gemini(prompt: str) -> str:
    try:
        ...
        return text[:200]  # ensure it never exceeds Scratch cloud variable limit
    except Exception as e:
        return f"Scratch => An error occurred: {e}"


@events.event
def on_set(activity):
    if activity.var == "prompt":
        print("Scratch => New prompt received.")
...
@events.event
def on_ready():
    print("Scratch => Event listener ready!")
